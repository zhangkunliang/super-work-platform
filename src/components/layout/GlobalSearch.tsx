...
export function GlobalSearch({ value, results, onChange, onSelect }: GlobalSearchProps) {
  ...
  const [selection, setSelection] = useState<{ key: string; index: number }>(() => ({ key: `${value}::${results.length}`, index: -1 }))
  const listKey = `${value}::${results.length}`
  const activeIndex = selection.key === listKey ? selection.index : -1
  const nextIndex = (current: number, delta: number) => (current + delta + visibleResults.length) % visibleResults.length

  function handleKeyDown(event: React.KeyboardEvent<HTMLInputElement>) {
    if (event.key === "ArrowDown" && visibleResults.length > 0) {
      event.preventDefault()
      setSelection((current) => ({ key: listKey, index: nextIndex(current.key === listKey ? current.index : -1, 1) }))
    } else if (event.key === "ArrowUp" && visibleResults.length > 0) {
      event.preventDefault()
      setSelection((current) => ({
        key: listKey,
        index: nextIndex(current.key === listKey ? current.index : 0, -1),
      }))
  ...
  return (
    <div className="relative w-full max-w-sm">
      ...
      <Input
        ...
        value={value}
        onChange={(event) => {
          setSelection({ key: `${event.target.value}::${results.length}`, index: -1 })
          onChange(event.target.value)
        }}
        onKeyDown={handleKeyDown}
        placeholder="搜索学生、任务或消息"
        className="h-11 border-white/10 bg-white/[0.06] pl-9 text-sm text-white placeholder:text-white/35 focus-visible:border-white/30 focus-visible:ring-white/20"
      />
